...
describe('#json handling', function(){
    it('should create vatsimData.json', async function(){
        handler = new DataHandler(overwrite = true);
        await handler.update();
        expect(json).to.be.a.jsonFile();
    });
    it('should store the last update date in the json file', function(){
        const file = fs.readFileSync(json);
        const parsed = JSON.parse(file)
        expect(new Date(parsed['updated_date'])).to.be.below(new Date())
    })
    it('should contain the connected pilots and controllers', function(){
        const file = fs.readFileSync(json);
        const parsed = JSON.parse(file)
